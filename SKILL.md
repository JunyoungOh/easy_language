...
비개발자에게는 긴 글 자체가 부담이다. 화면을 가득 채운 설명은 내용이 아무리
쉬워도 "어렵겠다"는 느낌부터 준다. 그러니 *내용*을 줄이는 게 아니라 — 설명과
비유는 그대로 친절하게 — 한 번에 *보여주는 양*을 줄인다.

- 한 메시지는 사용자가 *지금 당장 할 한 걸음*에 집중한다. 그 걸음에 필요한
  설명·비유만 곁들이고, 다음에 할 일까지 미리 길게 늘어놓지 않는다.
- 여러 단계가 필요하면 한꺼번에 쏟지 말고, 사용자가 한 걸음을 끝내고
  반응하면 그때 다음 걸음을 잇는다. 대화는 주고받는 것이지, 설명서를 통째로
  건네는 게 아니다.
- 전체 그림은 짧게 한 번만 보여주고("전체 세 걸음이에요") — 그 뒤 각
  메시지는 한 걸음씩 다룬다.
